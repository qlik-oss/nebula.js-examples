        ...
        {rows.map((row, rowIdx) => (
          <tr key={rowIdx}>
            {row.map((cell, colIdx) => (
              <td
                style={{
                  background: selectedElements.includes(cell.qElemNumber)
                    ? accent
                    : null,
                  color: selectedElements.includes(cell.qElemNumber)
                    ? fg
                    : null,
                }}
                onClick={() =>
                  canSelect &&
                  doSelection(selections, cell, colIdx, setSelectedElements)
                }
                key={`${rowIdx}-${colIdx}`}
              >
                {cell?.qText || ""}
  ...
  return (
    ...
    >
      <div className="abc-advanced-chart__meta">
        ...
        <span>
          Selections:{" "}
          {layout?.qSelectionInfo?.qInSelections ? "active" : "inactive"}
        </span>
        <span>
          Interactions:{" "}
          {interactions?.active && interactions?.select
            ? "enabled"
            : "disabled"}
        </span>
      </div>
      ...
      <div className="abc-advanced-chart__body">
        {datasetError ? (
          <div className="abc-advanced-chart__error">
            Data error: {String(datasetError)}
          </div>
        ) : (
          <TableView
            rows={dataset?.rows || []}
